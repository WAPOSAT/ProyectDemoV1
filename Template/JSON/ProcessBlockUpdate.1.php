...
While($PB1 = $si1->retornar_SELECT()){
    ...
    While($PB2 = $si1b->retornar_SELECT()){
        ...
        $new2 = array('Id'=>$PB2["id_sensor"], 'Name'=>$PB2["codename"]."p", 'Code'=>$PB2["codename"], 'Unit'=>"ppm", 'LastValue'=>$last["value"], 'LastDate'=>$last["date"] );
    ...
    $new = array('Id'=>$PB1["id_block"], 'NumDanger'=>$numDanger, 'NumRisk'=>$numRisk, 'Sensor'=>$Sensor );
    array_push($StationBlock, $new);
}

$StaticInfo = array('Id'=>$ProcessBlock["id_block"],'StationBlock'=>$StationBlock);
